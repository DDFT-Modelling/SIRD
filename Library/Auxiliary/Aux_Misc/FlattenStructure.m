...
%
    ...
    for i=1:length(names)     
        ...
        if(size(A.(names{i}),2)>1 && isstruct(A.(names{i})(1))) % deal with structure arrays
            nStruct = size(A.(names{i}),2);
            for iStruct = 1:nStruct
                newname = [names{i} '_' num2str(iStruct)];
                C.(newname) = A.(names{i})(iStruct);
            end
        elseif(~isstruct(A.(names{i})))
            C.(names{i}) = A.(names{i});
        else
