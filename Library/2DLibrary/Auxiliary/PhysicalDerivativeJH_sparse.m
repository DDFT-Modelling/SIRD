function Diff = PhysicalDerivativeJH(Pts,Diff1,Diff2,J,dH1,dH2)
    ...
    Diff.DDy1   = sparse(Hy(:,:,1,1));
    Diff.Dy1Dy2 = sparse(Hy(:,:,1,2));
    Diff.DDy2   = sparse(Hy(:,:,2,2));
            
    %Diff.DDy1(markInf,:,:) = 0;
    
    Diff.Lap    = sparse(Hy(:,:,1,1) + Hy(:,:,2,2));
    Diff.div    = sparse([Diff.Dy1 , Diff.Dy2]);
    Diff.LapVec = sparse([Diff.Lap , zeros(N1*N2); zeros(N1*N2) , Diff.Lap]);
    Diff.gradDivVec = sparse([Diff.DDy1 , Diff.Dy1Dy2 ; Diff.Dy1Dy2 , Diff.DDy2]);
