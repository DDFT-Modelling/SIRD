    ...
    methods
        ...
        function Interp = ComputeInterpolationMatricesPhys(this,Pts)
            ...
            for iShape = 1:this.nShapes
                ...
                % only keep those which came from the shape
                keepMask = (X1>=x1Min) & (X1<=x1Max) & (X2>=x2Min) & (X2<=x2Max);
                X1_keep = X1(keepMask);
                X2_keep = X2(keepMask);
                Y1_keep = Y1(keepMask);
                Y2_keep = Y2(keepMask);
                ...
                Y1_cut = [Y1_cut;Y1_keep]; %#ok
                Y2_cut = [Y2_cut;Y2_keep]; %#ok
                InterPol = blkdiag(InterPol,InterPolShape);

                % remove points already allocated to prevent them being 
                % allocated to be interpolated from more than one shape
