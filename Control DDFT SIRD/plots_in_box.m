...
if nargin <= 8
  filename = strcat( 'SIRD-DDFT-Forward.gif' );
end
    ...
    MaxS = max(S_t,[],'all');   MinS = min(S_t,[],'all')-1e-3;
    MaxI = max(I_t,[],'all');   MinI = min(I_t,[],'all')-1e-3;
    MaxR = max(R_t,[],'all');   MinR = min(R_t,[],'all')-1e-3;
